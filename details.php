...
class Module_Events_manager extends Module {
	...
	public function upgrade($old_version)
	{
        ...
        if($old_version == '1.0.2')
        {
            ...
            // Move settings
            $listLayoutSetting = $this->db->from('settings')->where('slug', 'em_list_layout')->get()->row();

            if($listLayoutSetting)
            {
                $listLayout = $listLayoutSetting->value;
            }

            else
            {
                $listLayout = 'default.html';
            }
